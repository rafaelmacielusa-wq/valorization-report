...
/**
 * ATUALIZAÇÃO SEGUNDO PEDIDOS
 * 1) Logos permanentes (sem upload): Diferencial e Bicalho de /public/logos.
 * 2) PDF estável e alinhado com A4 retrato (ajuste de escala para caber, sem tooltip no gráfico durante exportação).
 * 3) Paleta fixa (sem seletor de cores): Azul Bicalho + Laranja Diferencial.
 * 4) Mantidos dados de exemplo e casos de teste.
 */
...
function BrandLogo({ kind, alt }) {
  // Correção: remover uso de new URL com base inválida em sandbox; somente strings seguras
  ...
  const srcs = [
    ...
    ...(kind === "dif"
      ? ["/LOGO%20DIFERENCIAL.png", "/LOGO%20DIFERENCIAL.jpg", "/diferencial.png"]
      : ["/LOGO%20BICALHO.png", "/LOGO%20BICALHO.jpg", "/bicalho.png"]),
  ].filter(Boolean);
  const src = srcs[idx];
...
export default function ValorizationReportApp() {
  ...
  const exportPDF = async () => {
    try {
      setIsExporting(true); // esconde tooltip
      ...
      try {
        ...
        await pdf.html(reportRef.current, {
          ...
          callback: (doc) => doc.save(`relatorio_valorizacao_${(cliente || "cliente").replaceAll(" ", "_")}_${reportDate}.pdf`),
        });
      } catch (err2) {
  ...
  return (
    <div className="app">
      <style>{`
        :root { --accent1:${BRAND_ORANGE}; --accent2:${BRAND_BLUE}; --ink:#0F172A; }
        .app { min-height:100vh; width:100%; background:#FAFAFA; color:var(--ink); padding:24px; font-family: ui-sans-serif, system-ui, -apple-system, Segoe UI, Roboto, Arial, "Noto Sans", "Helvetica Neue"; }
        ...
        .muted { color:#6B7280; }
        .danger { color:#DC2626; }
        .title { font-weight:800; letter-spacing:0.04em; font-size:32px; }
      ...
      {/* Controles essenciais (sem upload e sem seleção de cores) */}
      ...
      {/* Área de Relatório para o PDF */}
      <div ref={reportRef} className="container report a4">
        <div className="brandbar" />

        {/* Cabeçalho com logos fixas e título central */}
        ...
        <div className="grid grid-2" style={{ gap: 12, marginTop: 16 }}>
          <div><div className="muted text-sm">Cliente</div><div title={cliente || ""}>{cliente || "–"}</div></div>
          <div><div className="muted text-sm">Total de Unidades</div><div>{totals.totalUnidades}</div></div>
          <div><div className="muted text-sm">Valor Total de Contratos (R$)</div><div>{currencyBR(totals.valorTotalContratos)}</div></div>
          <div><div className="muted text-sm">Relatório do dia</div><div>{formatDateBR(reportDate)}</div></div>
        </div>
        ...
        <div className="chart mt-6">
          <ResponsiveContainer>
            <PieChart>
              <Pie data={pieData} dataKey="value" nameKey="name" innerRadius={80} outerRadius={110} paddingAngle={2} isAnimationActive={!isExporting}>
