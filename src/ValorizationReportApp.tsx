// ===== Relatório De Valorização – SPA React (Premium) =====
...
export default function ValorizationReportApp() {
  ...
  const addEdgeCases = () => {
    ...
    const extras = [
      ...
      },
      {
        ...
        dataAquisicao: ymd(
          new Date(new Date(repDate).getTime() + 5 * 24 * 3600 * 1000)
        ),
  ...
  const parsedRows = useMemo(() => {
    ...
    return rows.map((r) => {
      ...
      let dias = 1;
      if (dAq && repDate && dAq <= repDate)
        dias = Math.max(1, Math.floor(((repDate as any) - (dAq as any)) / (1000 * 60 * 60 * 24)));

      const valorizacaoPct =
        va > 0 && isFinite(vc / va) ? (vc / va - 1) * 100 : NaN;
  ...
  const exportCSV = () => {
    ...
    const sep = ";";
    const csv = [header.join(sep), ...lines.map((l) =>
  ...
  const exportXLSX = () => {
    ...
    XLSX.writeFile(
      wb,
      `tabela_relatorio_valorizacao_${(cliente || "cliente").replaceAll(
        " ",
        "_"
      )}_${reportDate}.xlsx`
    );
  ...
  const exportPDF = async () => {
    try {
      ...
      pdf.save(
        `relatorio_valorizacao_${(cliente || "cliente").replaceAll(
          " ",
          "_"
        )}_${reportDate}.pdf`
      );
      ...
      try {
        const pdf = new jsPDF({ orientation: "portrait", unit: "mm", format: "a4" });
        await (pdf as any).html(reportRef.current, {
  ...
  return (
    <div className="app">
      <style>{`
        ...
        /* Tabela do RELATÓRIO (alto contraste final) */
        .tableWrap { overflow:auto; border:1px solid #E5E7EB; border-radius:16px; }
        table { width:100%; border-collapse:separate; border-spacing:0; font-size:13px; }
        thead { background:#0F172A; color:#FFFFFF; }
        th, td { padding:12px; text-align:left; border-top:1px solid #000; vertical-align:top; }
      ...
      <div className="container mb-6">
        ...
        <div className="row mt-6" style={{ flexWrap: "wrap", gap: 8 }}>
          ...
          <button onClick={addSamples} className="btn secondary">
            Adicionar 4 unidades de exemplo
          </button>
          <button onClick={addEdgeCases} className="btn secondary">
            Adicionar casos de teste (edge)
          </button>
                    ...
                    <tr key={row.id}>
                      ...
                      <td>
                        <input
                          ...
                          onChange={(ev) =>
                            handleRowChange(row.id, "valorAquisicao", maskBRL(ev.target.value))
                          }
                          placeholder="R$ 0,00"
                        />
                      </td>
      ...
      <div ref={reportRef} className="container report a4">
          ...
          <div className="chart">
            <ResponsiveContainer>
              <PieChart>
                ...
                >
                  {pieData.map((_, index) => (
                    <Cell key={index} fill={index === 0 ? BRAND_BLUE : BRAND_ORANGE} />
                  ))}
                </Pie>
            ...
            >
              <div
                style={{
                  fontWeight: 800,
                  fontSize: 40,
                  color: totals.valorizacaoAtualPct < 0 ? "#DC2626" : BRAND_ORANGE
                }}
              >
                {percent2(totals.valorizacaoAtualPct)}
              </div>
        ...
        {/* Tabela final (alto contraste) */}
        <div className="mt-6">
          <div className="tableWrap" style={{ border: "1px solid #000", borderRadius: 16 }}>
            <table>
              <thead>
                <tr>
                ...
                {rows.map((row, idx) => {
                  ...
                  return (
                    <tr key={row.id}>
                      ...
                      <td className="num">
                        {isValid || isFinite(pr?.valorAquisicao as number)
                      ...
                      <td className="num">
                        {isValid || isFinite(pr?.valorAtual as number)
                          ? currencyBR(pr?.valorAtual as number)
                          : warn(e.valorAtual || "Obrigatório")}
                      </td>
                      <td className="num">
                        {isValid ? (
                          <span className={(pr!.valorizacaoPct as number) < 0 ? "danger" : ""}>
                            {percent2(pr!.valorizacaoPct as number)}
                          </span>
                        ) : (
                      ...
                      <td className="num">
                        {isValid ? (
                          <span className={(pr!.lucroMes as number) < 0 ? "danger" : ""}>
                            {currencyBR(pr!.lucroMes as number)}
                          </span>
                        ) : (
                          <span className="muted">–</span>
                        )}
                      ...
                      <td className="num">
                        {isValid ? (
                          <span className={(pr!.lucroPctMes as number) < 0 ? "danger" : ""}>
                            {percent2(pr!.lucroPctMes as number)}
                          </span>
                        ) : (
                          <span className="muted">–</span>
                        )}
      ...
      <div className="container helper">
        Regras aplicadas: ignora linhas incompletas, valor de aquisição precisa ser &gt; 0,
        datas de aquisição futuras são invalidadas. Dias corridos = max(1, data do
        relatório - data de aquisição). Percentuais e moedas com 2 casas.
      </div>
    </div>
  );
}
